                                    ...
                                    <div class="col-md-12">
                                       <div class="panel widget">
                                          <div class="panel-heading vd_bg-grey">
                                             <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> List Unit Type</h3>
                                          </div>
                                                ...
                                                {{--    @foreach($getAllUnitTypes as $unitType)
                                                   <tr class="odd gradeX">
                                                      ...
                                                      <td>{{ $unitType->unit_type_name }}</td>
                                                       <td>{{ $unitType->unit_type_localname }}</td>
                                                      <td class="center menu-action">
                                                         ...
                                                         <a data-original-title="edit" data-toggle="modal" data-target="#editModal" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bd-yellow vd_yellow" onclick="getEditData({{$unitType->id}})"> <i class="fa fa-pencil"></i> </a>
                                                         <form action="{{route('unit-type-delete',$unitType->id)}}" method="POST">  
                                                         {{csrf_field()}}
                                                         @method('DELETE')
                                        ...
                                        <script type="text/javascript">
                                          ...
                                          function getEditData(id)
                                          {
                                           ...
                                           'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                             ...
                                             var action = "update-unit-type/"+id;
                                             
                                              $.ajax({

                                                type:"POST",
                                                url:"get-unit-type-view/"+id,
                                                data:{"_token": "{{ csrf_token() }}",'id':id},
                                                success:function(response)
                                                {
                                                   //alert(response);
                                                   $('.modal-body').html(` <form class="form-horizontal" method="POST"  enctype="multipart/form-data" action="`+action+`">
                                                      {{csrf_field()}}
                                                      
                                                      <div class="form-group">
                                                         <label class="col-sm-4 control-label">Unit Type Name</label>
                                                         <div class="col-sm-7 controls">
                                                            <input class="input-border-btm" type="text" name="unit_type_name" value="`+response.unit_type_name+`">
                                                         </div>
                                                      </div>
                                                       <div class="form-group">
                                                         <label class="col-sm-4 control-label">Unit Type  LocalName</label>
                                                         <div class="col-sm-7 controls">
                                                            <input class="input-border-btm" type="text" name="unit_type_localname" value="`+response.unit_type_localname+`">
                                                         </div>
                                                      </div>
